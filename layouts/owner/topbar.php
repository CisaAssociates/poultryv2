...
<div class="navbar-custom">
    <div class="topbar">
        ...
        <ul class="topbar-menu d-flex align-items-center">
            ...
            // Use the farms data already loaded in the user object instead of making a new query
            if (isset($user['farms']) && is_array($user['farms'])) {
                $farms = $user['farms'];
            } else {
                // Fallback to database query if farms not in user object
                $conn = db_connect();
                $farm_result = $conn->prepare("SELECT farm_id, farm_name FROM farms WHERE owner_id = ?");
            ...
            // Get current farm_id from session or set to first farm
            $current_farm_id = $_SESSION['selected_farm_id'] ?? null;

            // If no farm is selected yet, select the first one
            if (!$current_farm_id && count($farms) > 0) {
                $current_farm_id = $farms[0]['farm_id'];
                $_SESSION['selected_farm_id'] = $current_farm_id;
            }

            // Get current farm name for display
            $current_farm_name = "Select Farm";
            ...
            <?php if (count($farms) > 0): ?>
                <li class="dropdown">
                    <a class="nav-link dropdown-toggle waves-effect waves-light arrow-none" data-bs-toggle="dropdown" href="#" role="button" aria-haspopup="false" aria-expanded="false">
                        <i class="fe-home font-18 align-middle me-1"></i>
                        <span><?= special_chars($current_farm_name) ?></span>
                    </a>
                    <div class="dropdown-menu dropdown-menu-animated">
                        <?php foreach ($farms as $farm): ?>
                            <a href="<?= view('owner.switch-farm') ?>?farm_id=<?= $farm['farm_id'] ?>" class="dropdown-item">
                                <i class="fe-check-circle font-16 me-1 <?= $farm['farm_id'] == $current_farm_id ? 'text-success' : 'text-muted' ?>"></i>
                                <?= special_chars($farm['farm_name']) ?>
                            </a>
                        <?php endforeach; ?>
                        <div class="dropdown-divider"></div>
                        <a href="<?= view('owner.farms') ?>" class="dropdown-item">
                            <i class="fe-settings font-16 me-1"></i>
                            Manage Farms
                        </a>
                    </div>
                </li>
